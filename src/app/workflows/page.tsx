    ...
    <div className="min-h-screen bg-black text-white p-4 sm:p-8 font-sans relative">
      ...
      <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 border-b border-zinc-900 pb-6">
        ...
        <div className="flex items-center gap-3 self-end md:self-auto">
          <button
            onClick={() => state.dispatch(workflowsActions.fetchRequest())}
      ...
      {state.success && (
        <div className="mb-6 flex items-center gap-2.5 rounded-lg border border-emerald-950 bg-emerald-950/30 px-4 py-3.5 text-xs text-emerald-400 animate-slideDown">
          ...
          <span>Workflow action successful.</span>
        </div>
      )}
      ...
      {state.error && (
        <div className="mb-6 flex items-center gap-2.5 rounded-lg border border-red-955 bg-red-955/30 px-4 py-3.5 text-xs text-red-400 animate-slideDown">
      ...
      <div className="w-full overflow-hidden rounded-xl border border-zinc-900 bg-zinc-955/20 backdrop-blur-md">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="border-b border-zinc-900 bg-zinc-955/50 text-[10px] uppercase tracking-widest text-zinc-505 font-semibold">
              ...
              {state.filteredItems.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-5 py-12 text-center text-xs text-zinc-500">
                    {state.loading ? "Syncing workflow automation records..." : "No active workflows match the search criteria."}
                  </td>
                </tr>
              ) : (
                state.filteredItems.map((item) => (
                  <tr key={item.id} className="group hover:bg-zinc-900/10 transition-all duration-300 border-b border-zinc-900">
                    ...
                    <td className="px-5 py-4">
                      <span className={`inline-flex items-center rounded-md border px-2 py-0.5 text-[10px] font-mono font-semibold uppercase ${
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg border border-zinc-800 hover:border-zinc-755 hover:bg-red-955/20 hover:text-red-400 transition-all text-zinc-400 cursor-pointer"
                        >
      ...
      {state.isRunsModalOpen && state.runsWorkflow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm transition-opacity duration-300" onClick={() => state.dispatch(workflowsActions.setFormField({ field: "isRunsModalOpen", value: false }))} />
          ...
          <div className="relative w-full max-w-lg rounded-2xl border border-zinc-800 bg-zinc-950 p-6 shadow-2xl shadow-black/90 text-white animate-scaleIn">
            <button
              onClick={() => state.dispatch(workflowsActions.setFormField({ field: "isRunsModalOpen", value: false }))}
            ...
            <h2 className="text-lg font-light tracking-tight mb-1 flex items-center gap-2">
              ...
              Runs — {state.runsWorkflow.name}
            </h2>
            <p className="text-[11px] text-zinc-500 mb-4">Workflow #{state.runsWorkflow.id}</p>

            <button
              onClick={() => state.handleTriggerRun(state.runsWorkflow!.id)}
                ...
                state.runs.map((run) => (
                  <div key={run.id} className="p-3.5 flex items-center justify-between text-xs">
                    <div>
                      ...
                      <span className="ml-2 text-zinc-505">
      ...
      {state.isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm transition-opacity duration-300" onClick={() => state.dispatch(workflowsActions.closeModal())} />
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-950 p-6 shadow-2xl shadow-black/90 text-white animate-scaleIn">
            <button
              onClick={() => state.dispatch(workflowsActions.closeModal())}
              className="absolute right-5 top-5 p-1 rounded-full text-zinc-500 hover:text-white hover:bg-zinc-900 transition-all duration-300 cursor-pointer"
            >
              <X className="h-4 w-4" />
            </button>
            ...
            <h2 className="text-lg font-light tracking-tight mb-6">
              {state.modalMode === "create" ? "Configure Workflow" : "Update Workflow Definition"}
            </h2>
            ...
            <form onSubmit={state.handleSubmit} className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  value={state.name}
                  onChange={(e) => state.dispatch(workflowsActions.setFormField({ field: "name", value: e.target.value }))}
                  placeholder="e.g. Q3 Outbound Invoice Approvals"
                  required
                  className="w-full rounded-lg bg-zinc-900/60 border border-zinc-800 p-2.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-zinc-700 transition-all duration-300 focus:border-zinc-750"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <select
                  value={state.status}
                  onChange={(e) => state.dispatch(workflowsActions.setFormField({ field: "status", value: e.target.value }))}
                  className="w-full rounded-lg bg-zinc-900/60 border border-zinc-800 p-2.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-zinc-700 transition-all duration-300 focus:border-zinc-750"
                >
