...
export default function ServiceResourcesPage() {
  const state = useServiceResourcesPageState();
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 sm:p-8 font-sans relative">
      ...
      <div className="flex items-center gap-2 text-xs text-zinc-500 mb-4">
        ...
        <span>/</span>
        <span className="text-zinc-300">Service Resources</span>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-1">
                Administer technicians, equipment dispatch crews, and custom capability profiles.
              </p>
            </div>
          </div>
        </div>
                    ...
                    <td className="px-5 py-4 text-right pr-6">
                      <div className="flex items-center justify-end gap-2.5">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg border border-zinc-800 hover:border-red-900/50 hover:bg-red-950/20 bg-zinc-955/60 hover:text-red-400 transition-all text-zinc-400 cursor-pointer"
                        >
      ...
      {state.isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm transition-opacity duration-300" onClick={() => state.dispatch(resourcesSlice.actions.closeModal())} />
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-950 p-6 shadow-2xl shadow-black/90 text-white animate-scaleIn">
            <button
              onClick={() => state.dispatch(resourcesSlice.actions.closeModal())}
            ...
            <h2 className="text-lg font-light tracking-tight mb-5 flex items-center gap-2">
              ...
              {state.modalMode === "create" ? "Add Service Resource" : "Edit Service Resource"}
            </h2>
            ...
            <form onSubmit={state.handleSubmit} className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  value={state.name}
                  onChange={(e) => state.dispatch(resourcesSlice.actions.setFormField({ field: "name", value: e.target.value }))}
                  placeholder="e.g. John Doe (HVAC Lead)"
                  required
              ...
              <div className="space-y-1">
                <label className="text-[10px] text-zinc-500 uppercase tracking-widest block font-medium">Resource Type</label>
                <select
