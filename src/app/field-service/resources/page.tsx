...
import { Search, Users, Plus, RefreshCw, CheckCircle2, AlertCircle, X, Edit2, Trash2, ShieldAlert } from "lucide-react";
...
export default function ResourcesPage() {
  ...
  const [resourceType, setResourceType] = useState<"technician" | "crew">("technician");
  ...
  const openCreateModal = () => {
    setModalMode("create");
    setName("");
    setResourceType("technician");
    setIsActive(true);
    setIsModalOpen(true);
  };

  const openEditModal = (item: ServiceResource) => {
    setModalMode("edit");
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-1">
                Manage dispatchable field agents, technical specialists, and service crew teams.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-24 text-zinc-500">
              ...
              <p className="text-xs tracking-wider">Syncing service resources...</p>
            </div>
          ) : (
            <table className="w-full min-w-[700px] text-sm text-left">
              <thead>
                <tr className="bg-zinc-950/80 text-left text-[11px] text-zinc-400 uppercase tracking-wider border-b border-zinc-800">
                  <th className="px-5 py-4 font-medium w-24">Resource ID</th>
                  <th className="px-5 py-4 font-medium w-72">Name</th>
                  <th className="px-5 py-4 font-medium w-48">Type</th>
              ...
              <tbody>
                {filteredItems.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-xs font-mono text-zinc-400 capitalize">
                      {item.resourceType}
                    </td>
                ...
                {filteredItems.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-5 py-16 text-center text-zinc-500 text-xs">
                      No service resources found in directory.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-950 p-6 shadow-2xl shadow-black/90 text-white animate-scaleIn">
            ...
            <h2 className="text-lg font-light tracking-tight mb-5 flex items-center gap-2">
              ...
              {modalMode === "create" ? "Add Service Resource" : "Edit Service Resource"}
            </h2>

            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] text-zinc-500 uppercase tracking-widest block font-medium">Resource Name</label>
                <input
                  type="text"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="e.g. Sarah Connor (HVAC Specialist)"
                  required
              ...
              <div className="space-y-1">
                <label className="text-[10px] text-zinc-500 uppercase tracking-widest block font-medium">Resource Classification</label>
                <select
                  value={resourceType}
                  onChange={(e) => setResourceType(e.target.value as "technician" | "crew")}
                  className="w-full rounded-lg bg-zinc-900/60 border border-zinc-800 p-2.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-zinc-700 transition-all duration-300"
                >
                  <option value="technician">Technician (Individual field agent)</option>
                  <option value="crew">Service Crew (Group/Equipment Team)</option>
                </select>
              </div>
              ...
              <div className="flex items-center gap-2.5 pt-2">
                ...
                <label htmlFor="isActive" className="text-xs text-zinc-300 select-none cursor-pointer">
                  Resource is active & dispatchable on scheduling requests
                </label>
              </div>
