...
export default function OperatingHoursPage() {
  const state = useOperatingHoursPageState();
    ...
    <div className="min-h-screen bg-black text-white p-4 sm:p-8 font-sans relative">
                    ...
                    <td className="px-5 py-4 text-right pr-6">
                      <div className="flex items-center justify-end gap-2.5">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg border border-zinc-800 hover:border-red-900/50 hover:bg-red-955/20 bg-zinc-950/60 hover:text-red-400 transition-all text-zinc-400 cursor-pointer"
                        >
      ...
      {state.isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm transition-opacity duration-300" onClick={() => state.dispatch(operatingHoursSlice.actions.closeModal())} />
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-950 p-6 shadow-2xl shadow-black/90 text-white animate-scaleIn">
            <button
              onClick={() => state.dispatch(operatingHoursSlice.actions.closeModal())}
            ...
            <h2 className="text-lg font-light tracking-tight mb-5 flex items-center gap-2">
              ...
              {state.modalMode === "create" ? "Add Operating Hours" : "Edit Operating Hours"}
            </h2>
            ...
            <form onSubmit={state.handleSubmit} className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  value={state.name}
                  onChange={(e) => state.dispatch(operatingHoursSlice.actions.setFormField({ field: "name", value: e.target.value }))}
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => state.dispatch(operatingHoursSlice.actions.setFormField({ field: "timezone", value: e.target.value }))}
                  className="w-full rounded-lg bg-zinc-900/60 border border-zinc-800 p-2.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-zinc-700 transition-all duration-300"
                >
