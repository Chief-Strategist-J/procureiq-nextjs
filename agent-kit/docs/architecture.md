...
## 15. Why "the AI isn't following the rules" happens — and the actual fix

Here's the brutal truth: **a prose document, no matter how well-written, is advisory to an LLM, not enforced.** An AI reading 400 lines of markdown will drift, skip sections under time/token pressure, "interpret" a MUST as a SHOULD, or simply forget a rule from earlier in a long generation. This isn't a prompting failure you can fix by writing the rules more forcefully in English — it's a structural problem, and it needs a structural fix. If you're only pointing an AI agent at this document and hoping it complies, it will not comply reliably, ever, at any length of document. Below is what actually closes that gap.

### 15.1 — Prose rules are not real rules until something mechanical checks them

Every rule in this document falls into one of two buckets, and you must know which bucket each one is in:

- **Bucket A — mechanically checkable.** Can be verified by a linter, a static analyzer, a regex grep, a type checker, a schema validator, a CI script — with zero judgment required. Examples: "no `!!` in Kotlin," "no `any` in TypeScript," "every migration is additive," "every mutating endpoint requires an Idempotency-Key header," "no `fetch` inside a component file." **These are not requests to an AI. These are automated gates that reject the change before a human or an AI ever gets to argue about it.**
- **Bucket B — requires judgment.** Things like "does this respect vertical-slice boundaries," "is this business logic that belongs in the rules engine." These genuinely need review (human or Agent 2's LLM pass, §10) — but even these should be narrowed toward Bucket A wherever possible over time (e.g., "no cross-feature imports" starts as a judgment call and becomes a mechanical import-boundary lint the first time someone violates it).

**The fix, stated as a rule itself: every rule in this document that CAN be expressed as a lint/static-check/schema-check MUST be, and the moment a violation happens once, it becomes an automated gate — it never remains "a thing we told the AI to remember."** If your AI keeps violating a rule, the rule was in Bucket B when it should have been moved to Bucket A. That is the concrete, actionable fix for "my AI isn't following the rules": stop re-explaining the rule in prompts, and go write the linter.

### 15.2 — A machine-readable rules manifest, not just this document

This document is for humans. AI agents (yours, Agent 1/2/3 from §10, or any coding assistant) must be pointed at a **machine-readable manifest**, not asked to parse markdown prose and infer intent:
...
rules:
  - id: KT-NULL-001
    ...
    check: "grep -Rn '!!' --include='*.kt' features/ domain/ adapters/ | grep -v '/test/'"
    doc_ref: "#3-brutal-type-safety-null-safety-empty-invalid-data-rules"
  - id: TS-ANY-001
    lang: typescript
    severity: blocking
    statement: "No `any` type outside explicitly justified escape hatches."
    check: "eslint --rule '@typescript-eslint/no-explicit-any: error'"
    doc_ref: "#3-brutal-type-safety-null-safety-empty-invalid-data-rules"
  - id: DB-MIGRATION-001
    lang: any
    severity: blocking
    statement: "No NOT NULL column added without a default in the same migration."
    check: "scripts/lint-migrations.sh"
    doc_ref: "#12-database-layer"
  - id: API-IDEMPOTENCY-001
    lang: any
    severity: blocking
    statement: "Every mutating (POST/PUT/PATCH/DELETE) endpoint's contract requires an Idempotency-Key."
    check: "scripts/lint-openapi-idempotency.sh"
    doc_ref: "#14-the-full-identifier-taxonomy"
  - id: UI-LOGIC-001
    lang: typescript
    severity: blocking
    statement: "No business conditional (>1 boolean condition) inside JSX."
    check: "eslint custom rule: no-business-logic-in-jsx"
    doc_ref: "#11-frontend"
  - id: NAMING-001
    lang: any
    severity: blocking
    statement: "No literal example nouns copied from architecture docs/templates."
    check: "scripts/validate-naming.sh"
    doc_ref: "#0-the-naming-law"
```

Every rule gets an **ID, a severity, an exact mechanical check command, and a doc reference.** This is what Agent 1/2/3 (§10) actually consume programmatically — not "please read the architecture doc and follow it," but "run `platform/rules-manifest.yaml`'s checks, and every `severity: blocking` finding is a hard stop, not a suggestion." An AI given a list of exact shell commands to run and a pass/fail result is enormously more reliable than an AI given prose and asked to self-assess compliance.

### 15.3 — The pre-flight protocol every AI agent MUST run, non-negotiable, before emitting a single line of code

This is the direct fix for "my AI still isn't following the rules." Any AI agent (in-IDE assistant, Agent 1, a chat session) working in this codebase follows this exact sequence, every single time, with no shortcuts:

1. **Load `platform/rules-manifest.yaml` and every linked ADR in `platform/adr/` fresh — never rely on anything recalled from earlier in the session.**
2. **State, before writing any code, which rule IDs apply to the task at hand** (e.g., "this touches a mutating endpoint → API-IDEMPOTENCY-001 applies; this touches a component → UI-LOGIC-001 applies").
3. **Write the code.**
4. **Run every applicable mechanical check locally before presenting the result** — not "I believe this follows the rules," but the literal command output. If a check tool isn't runnable in the current environment, the agent explicitly states that and flags the output as unverified, rather than silently asserting compliance.
5. **If ANY blocking rule cannot be satisfied, the agent STOPS and surfaces the conflict explicitly — it does not silently ship a violation, and it does not silently "interpret" the rule to make the conflict disappear.** A rule conflict is a decision point requiring a human or an ADR supersession (§10), never a judgment call an AI resolves quietly on its own.
6. **Only after step 4 passes clean does the agent present the change as ready.**

**If your AI is skipping straight to step 3 — writing code first and never running or reporting the mechanical checks — that is the exact failure mode causing your problem, and the fix is to make steps 1–2 and 4–5 a hard, explicit part of every request you give it, not an assumed background process.** Put bluntly: tell it "before you write anything, list the rule IDs that apply and show me the check commands you'll run after," every single time, until it's habitual — an AI that's never asked to show its check output will never reliably produce it unprompted.

### 15.4 — Zero trust in AI self-report, always

**No agent's claim of "this follows the rules" is ever taken as true without independent verification — this applies to Agent 1, Agent 2, Agent 3, and any human-facing AI assistant equally.** The actual source of truth is:

- Git pre-commit hooks running the Bucket A checks locally, before a commit is even possible.
- CI (§8) re-running the exact same checks server-side — a pre-commit hook can be bypassed (`--no-verify`); CI cannot.
- A merge is blocked by CI status, not by an agent's self-assessment, not by a reviewer's trust in the agent, not by anything short of the actual check passing.

**If your AI says "done, follows all rules" and you merge on that basis without the CI gate actually running and passing, you have not enforced anything — you've just trusted a probabilistic text generator's opinion of its own output, which is precisely how rule violations get through despite a 400-line document telling it not to.**

### 15.5 — Escalating rule severity so "non-negotiable" actually means something distinct from "important"

Not everything can be `severity: blocking` or the manifest becomes noise and gets ignored wholesale. Three tiers, strictly:

- **`blocking`** — violates data integrity, security, tenant isolation, or type safety. Fails CI. Fails pre-commit. No override, no exception, no "just this once" — not even from a senior engineer, not even under a deadline. The only way past a `blocking` rule is changing the rule itself via an ADR (§10), never bypassing it for one change.
- **`required-with-justification`** — should always be followed, but a rare, explicit, documented exception is possible (e.g., a genuinely justified `any` at a hard third-party-library boundary). This requires an inline comment citing the rule ID and the reason, reviewed by a human, logged — not a silent pragma-disable.
- **`advisory`** — best practice, surfaced as a warning, never blocks anything, tracked as tech debt.

**A rule with no tier is not a rule — every single rule anywhere in this document must be assigned one of these three tiers in the manifest before an AI agent can be expected to treat it correctly.** This is precisely what "must strictly follow each rule critically" requires structurally: the AI isn't given 60 rules of equal, vague weight — it's given an exact, machine-parseable severity per rule, and `blocking` violations are architecturally impossible to merge, not just discouraged.

---

## 16. More non-negotiable rules — security, dependencies, resource limits, deprecation, and operational discipline

### Security — blocking, always

- **No secret, credential, API key, or connection string is ever committed, hardcoded, or logged — this includes inside test fixtures, example configs, and AI-generated scaffolding.** A pre-commit secret-scanner (gitleaks or equivalent) runs on every commit; CI runs it again; a caught secret triggers immediate rotation, not just removal from the diff (a secret that touched git history is compromised, full stop, even if the commit is amended).
- **Every dependency addition (any language) is scanned for known CVEs before merge** (`govulncheck`, `npm audit`/`osv-scanner`, OWASP dependency-check for JVM) — a `blocking` finding on a critical/high CVE with a known fix available is not mergeable; a finding with no fix available requires an explicit, human-approved, time-boxed exception logged in an ADR.
- **Least privilege, everywhere, no exceptions:** service accounts, DB roles (§12), API scopes — each is granted exactly what the specific operation needs, never a broad "admin" credential used because it's convenient during development and never revisited.
- **All inter-service traffic is authenticated (mTLS or signed tokens) — "it's internal, on our network" is not a security boundary and is explicitly rejected as a justification anywhere in this system.**
- **Input from any external boundary (user input, third-party webhook, file upload) is treated as hostile by default** — this is the same principle as §3's type-safety boundary validation, extended explicitly to security: sanitize, validate, and never trust a payload's self-declared type/size/content without checking.

### Dependency management — blocking

- **Every dependency is pinned to an exact version (lockfiles committed: `go.sum`, `package-lock.json`/equivalent, Gradle lockfiles) — no floating version ranges in a committed manifest, ever.**
- **A new dependency requires justification: what does it replace, why isn't the ~40–80 lines of `platform/fp` (§5) or an existing dependency sufficient.** This is reviewed the same way a structural change is (Agent 2, §10) — dependency sprawl is an architecture decision, not a convenience.
- **License compliance is checked automatically** — a copyleft license incompatible with your distribution model is a `blocking` CI finding, not something discovered during a legal review months later.
- **Scheduled, automated dependency-freshness checks (already in §8's nightly pipeline) — a dependency more than N major versions behind, or with no updates in over a defined period, is flagged, tracked, and has an owner, not silently ignored indefinitely.**

### Resource limits & graceful degradation — blocking

- **Every service declares explicit CPU/memory requests AND limits in its deployment manifest — "no limit" is not a valid configuration for anything reaching production.**
- **Every external call (§4's HTTP client, every DB query, every broker publish) has an explicit timeout AND a circuit breaker with defined thresholds — sourced from centralized config (§6), never hand-set per call site.**
- **Backpressure is explicit, not implicit.** A queue/channel with unbounded capacity is a `blocking` finding — every buffer has a max size and a defined behavior when full (reject, shed oldest, block with timeout — chosen deliberately per use case, documented).
- **Graceful shutdown is mandatory for every service:** on a termination signal, stop accepting new work, finish in-flight requests within a bounded drain period, THEN exit — a service that drops in-flight requests on every deploy is producing silent data loss on a schedule (every deploy), not an edge case.
- **Load-shedding behavior is defined before it's needed, not improvised during an incident:** when a service is over capacity, it has an explicit policy (reject low-`Priority` requests first, per §14's Priority identifier) rather than degrading uniformly until it falls over entirely.

### API evolution & deprecation — blocking

- **Breaking changes require a new `API-Version` (§14) — a field type change, a required-field addition, or a removed field on an EXISTING version is `blocking`, full stop, no "it's a small change" exception.**
- **Every deprecation has an announced, config-driven sunset date** (`platform/config`, §6) — the deprecated version continues functioning, emits a deprecation warning header and a traced metric on every use, and is removed only after the sunset date passes AND telemetry confirms zero real traffic, not just after the date arrives on the calendar.
- **A consumer-facing breaking change without a corresponding contract-test update (§7/§13) is `blocking` — the contract test is the mechanical proof the change was intentional, not a doc comment claiming it was.**

### Observability budgets — required-with-justification

- **Every service defines an error-budget/SLO (from §8's perf-budgets.yaml) — a change that would knowingly burn the error budget requires explicit sign-off, tracked the same way a `required-with-justification` rule exception is (§15.5), not silently shipped.**
- **Alert fatigue is treated as a bug:** an alert that fires without ever requiring action within a defined observation window is disabled or fixed, not left "just in case" — noisy alerting trains people to ignore the signal that matters.

### Data lifecycle — blocking

- **Every table/collection storing personal or sensitive data has an explicit, documented retention period and a scheduled deletion/anonymization job — indefinite retention "because storage is cheap" is not a valid default.**
- **Data deletion requests (right-to-erasure or equivalent) are modeled as a first-class, traced operation across every store AND every derived copy/cache/read-model (§13's reconciliation principle applies here too) — a deletion that only touches the primary store while a cache or replica retains the data is a compliance failure, not a technical nuance.**
- **Backups are tested by actually restoring them on a schedule — an untested backup is not a backup, it's an assumption, and this game-day exercise is scheduled exactly like the replica-failover drill in §12.**

### The meta-rule that ties §15 and §16 together

**Every single rule added in this document — past, present, and future — must be assigned a severity tier (§15.5) and, wherever mechanically possible, a check command in `platform/rules-manifest.yaml` before it can be considered "adopted."** A rule that exists only as a paragraph of prose, with no linked check and no severity, is not yet a rule in this system — it's a proposal. This is the actual, structural answer to "my AI isn't following the rules": stop treating this document as instructions to an AI, and start treating it as a specification for a manifest of automated gates that no agent — human or AI — can route around.
